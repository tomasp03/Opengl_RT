...
float map(vec3 p)
{
	...
	p *= rotXY(vec2(u_time * .9, u_time * 0.6));
	const float w = 1.5;d
	vec3 q = abs(p);
...
float map(vec3 p)
{
	p *= rotXY(vec2(u_time * 0.6, u_time * .9));
...
float map(vec3 p)
{
	...
	p.y += sin(p.z * 0.1 + u_time) * (3. + sin(u_time) * 1.);
	p.y += sin(p.x * 0.5 + u_time) * (3. + sin(u_time) * 1.);
	return length(p.y) - 2.;
}
#endif
...
void main()
{
	lgt = vec3(4. + sin(u_time * 0.4) * 20., 4. + sin(u_time * .1) * 10., 5.);
	vec2 p = gl_FragCoord.xy / u_resolution.xy - 0.5;
	p.x *= u_resolution.x / u_resolution.y;
	vec2 bp = p;
	p *= 100. + sin(u_time * 0.5) * 80.;
	p.x += sin(u_time) * 10.;
	p.y += cos(u_time * 0.9 + 0.5) * 4.;

	float st = sin(u_time * 0.2) * 0.3 + 1.;
	vec3 col = vec3(0.);
	for (float i = -NUM_LAYERS / 2.; i < NUM_LAYERS / 2.; i++)
	{
		...
		vec3 colx = vec3(0.15 + cos(u_time + 0.1) * 0.1, 0.2, 0.3 + sin(u_time) * 0.1) * rz.x * 3.;
		//borders
